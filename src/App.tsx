...
const map: Record<Phase, { cls: string; color: string }> = {
    idle: { cls: "", color: "bg-[#4c1d95]" },
    starting: { cls: "dot-throb", color: "bg-[#7c3aed]" },
    recording: { cls: "dot-blink", color: "bg-[#a855f7]" },
    paused: { cls: "", color: "bg-[#6d28d9]" },
    sending: { cls: "dot-throb", color: "bg-[#7c3aed]" },
    processing: { cls: "dot-throb", color: "bg-[#6d28d9]" },
    speaking: { cls: "dot-blink", color: "bg-[#c4b5fd]" },
    voicePaused: { cls: "", color: "bg-[#c4b5fd]" },
  };
...
function ControlButton({ accent, icon, label, hint, onClick, disabled, active }: ControlButtonProps) {
  const hoverBorder: Record<Accent, string> = {
    coral: "hover:border-[#a855f7]/70 hover:shadow-[0_10px_34px_-14px_rgba(168,85,247,0.55)]",
    cyan: "hover:border-[#7c3aed]/70 hover:shadow-[0_10px_34px_-14px_rgba(168,85,247,0.5)]",
    mint: "hover:border-[#c4b5fd]/70 hover:shadow-[0_10px_34px_-14px_rgba(168,85,247,0.5)]",
    amber: "hover:border-[#6d28d9]/70 hover:shadow-[0_10px_34px_-14px_rgba(124,58,237,0.5)]",
  };
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={disabled}
      className={`ctrl-btn relative flex h-24 flex-col items-center justify-center gap-1.5 rounded-xl border bg-[#241339] px-2 outline-none sm:h-28 ${
        active ? "border-[#a855f7]/80" : "border-[#5b21b6]"
      } ${hoverBorder[accent]} focus-visible:ring-2 focus-visible:ring-[#7c3aed]/60`}
    >
      ...
      <span className="text-sm font-semibold tracking-wide text-[#f5f3ff]">{label}</span>
      <span className="font-mono-gem text-[10px] uppercase tracking-[0.14em] text-[#c4b5fd]">{hint}</span>
    </button>
  );
}
...
export default function App() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 pb-4 pt-6 sm:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="ctrl-btn grid h-10 w-10 place-items-center rounded-xl border border-[#5b21b6] bg-[#241339] text-[#c4b5fd] hover:border-[#7c3aed]/60 hover:text-[#f5f3ff]"
          >
      ...
      <div className="sticky top-0 z-30 border-y border-[#5b21b6] bg-[#241339]/85 backdrop-blur-md">
      ...
      <main className="mx-auto grid max-w-6xl gap-4 px-4 py-5 sm:px-6 lg:grid-cols-[1fr_360px]">
        ...
        <div className="flex min-w-0 flex-col gap-4">
          ...
          <section className="rounded-xl border border-[#5b21b6] bg-[#241339] p-4 sm:p-5">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                <StatusDot phase={phase} />
                <p className="text-sm font-medium text-[#f5f3ff]">{status}</p>
                {phase === "speaking" && <Equalizer />}
              </div>
              <div className="flex items-center gap-3 font-mono-gem text-xs text-[#c4b5fd]">
                <span className={phase === "recording" ? "text-[#a855f7]" : phase === "paused" ? "text-[#6d28d9]" : ""}>
                  {formatTime(elapsed)}
                </span>
                <span className="hidden rounded border border-[#5b21b6] px-1.5 py-0.5 text-[10px] sm:inline">
                  {formatBytes(clipBytes)}
                </span>
              </div>
            </div>
            <div className="overflow-hidden rounded-lg border border-[#5b21b6]/70 bg-[#2e1065]">
              <Waveform getAnalyser={getAnalyser} mode={waveMode} />
            </div>
            <div className="mt-3 grid grid-cols-3 gap-2 font-mono-gem text-[10px] uppercase tracking-widest text-[#c4b5fd]">
              <span>
                Canal <b className="text-[#f5f3ff]">mic</b>
              </span>
              <span className="text-center">
                Modo <b className="text-[#f5f3ff]">{hasAudio ? "acumular" : "reposo"}</b>
              </span>
              <span className="text-right">
                Clip <b className="text-[#f5f3ff]">{hasAudio ? "abierto" : "—"}</b>
              </span>
            </div>
          </section>

          {/* Área de texto: estado + respuesta de la IA */}
          <section className="flex-1 rounded-xl border border-[#5b21b6] bg-[#241339] p-4 sm:p-5">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="font-display text-xs font-semibold uppercase tracking-[0.2em] text-[#c4b5fd]">
                Respuesta del asistente
              </h2>
              {wordCount > 0 && (
                <span className="font-mono-gem text-[10px] uppercase tracking-widest text-[#c4b5fd]">
            ...
            {/* Error discreto */}
            {error && (
              <div className="answer-in mb-3 flex items-start gap-2.5 rounded-lg border border-[#a855f7]/40 bg-[#a855f7]/10 px-3.5 py-3 text-sm text-[#1a0b2e]">
            ...
            {phase === "processing" || phase === "sending" ? (
              <div className="answer-in space-y-3 py-2">
                {[92, 100, 78].map((w, i) => (
                  <div key={i} className="h-3 animate-pulse rounded-full bg-[#241339]" style={{ width: `${w}%` }} />
                ))}
                <p className="pt-1 font-mono-gem text-xs text-[#c4b5fd]">
                  {phase === "sending" ? "Codificando audio en Base64…" : "Gemini está escuchando tu clip…"}
                </p>
              </div>
            ) : response ? (
              <p className="answer-in whitespace-pre-wrap text-[15px] leading-relaxed text-[#f5f3ff]">{response}</p>
            ) : (
              <div className="py-6 text-center">
                <p className="mx-auto max-w-md text-sm leading-relaxed text-[#c4b5fd]">
                  Presiona <b className="text-[#a855f7]">Grabar</b> y formula tu pregunta en voz alta. Puedes{" "}
                  <b className="text-[#6d28d9]">pausar</b> para leer y <b className="text-[#a855f7]">reanudar</b>: todo se
                  acumula en un solo clip. Luego <b className="text-[#7c3aed]">Envía</b> y escucha la respuesta.
                </p>
              </div>
            )}
          </section>
        ...
        <div className="flex min-w-0 flex-col gap-4">
          ...
          <section className="rounded-xl border border-[#5b21b6] bg-[#241339] p-4 sm:p-5">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="flex items-center gap-2 font-display text-xs font-semibold uppercase tracking-[0.2em] text-[#c4b5fd]">
              ...
              {history.length > 0 && (
                <button
                  type="button"
                  onClick={() => setHistory([])}
                  className="ctrl-btn flex items-center gap-1.5 rounded-md border border-[#5b21b6] px-2 py-1 font-mono-gem text-[10px] uppercase tracking-widest text-[#c4b5fd] hover:border-[#c4b5fd]/50 hover:text-[#c4b5fd]"
                >
            ...
            {history.length === 0 ? (
              <p className="py-3 text-center font-mono-gem text-xs text-[#c4b5fd]/70">
                ...
                {history.map((item, i) => (
                  <li key={item.id}>
                    <button
                      ...
                      className="ctrl-btn w-full rounded-lg border border-[#5b21b6] bg-[#2e1065] px-3 py-2.5 text-left hover:border-[#a855f7]/50"
                      title="Escuchar de nuevo"
                    >
                      <span className="flex items-center justify-between font-mono-gem text-[10px] uppercase tracking-widest text-[#a855f7]">
                        <span>R{history.length - i} · {item.time}</span>
                        <PlayIcon size={11} className="text-[#c4b5fd]" />
                      </span>
                      <span className="mt-1 line-clamp-2 block text-xs leading-relaxed text-[#f5f3ff]">
                        {item.text}
                      </span>
                    </button>
                  </li>
          ...
          {/* Configuración */}
          <section className="rounded-xl border border-[#5b21b6] bg-[#241339] p-4 sm:p-5">
            <h2 className="mb-3 flex items-center gap-2 font-display text-xs font-semibold uppercase tracking-[0.2em] text-[#c4b5fd]">
              <KeyIcon size={15} /> Configuración
            </h2>

            <label className="mb-1 block font-mono-gem text-[10px] uppercase tracking-widest text-[#c4b5fd]" htmlFor="gem-mic">
            ...
            <div className="mb-2 flex gap-2">
              <select
                ...
                className="min-w-0 flex-1 rounded-lg border border-[#5b21b6] bg-[#241339] px-3 py-2 font-mono-gem text-xs text-[#f5f3ff] outline-none transition-colors focus:border-[#7c3aed]/60"
              >
              ...
              <button
                ...
                className="ctrl-btn rounded-lg border border-[#5b21b6] bg-[#241339] px-3 py-2 text-xs font-semibold text-[#c4b5fd] hover:border-[#7c3aed]/60 hover:text-[#f5f3ff]"
            ...
            {activeMicLabel && (
              <p className="mb-1 -mt-1 font-mono-gem text-[10px] uppercase tracking-widest text-[#a855f7]">
                â— {activeMicLabel}
              </p>
            )}
            {micDiagnostic && (
              <p className="mb-2 -mt-1 text-[11px] leading-relaxed text-[#6d28d9]">{micDiagnostic}</p>
            )}
            <p className="mb-3 -mt-1 text-[11px] leading-relaxed text-[#c4b5fd]">
            ...
            <label className="mb-1 block font-mono-gem text-[10px] uppercase tracking-widest text-[#c4b5fd]" htmlFor="gem-out">
            ...
            <div className="mb-3 flex gap-2">
              ...
              <button
                ...
                className="ctrl-btn rounded-lg border border-[#a855f7]/50 bg-[#a855f7]/10 px-3 py-2 text-xs font-semibold text-[#a855f7] hover:bg-[#a855f7]/20"
                title="Reproduce un sonido corto por el dispositivo elegido"
              >
                Probar
              </button>
            </div>
            <p className="mb-4 -mt-2 text-[11px] leading-relaxed text-[#c4b5fd]">
              Seleccioná los auriculares o el dispositivo Bluetooth/USB-C. La voz del
              asistente y los beeps de feedback saldrán por acá.
            </p>

            <label className="mb-1 block font-mono-gem text-[10px] uppercase tracking-widest text-[#c4b5fd]" htmlFor="gem-key">
              API Key de Gemini
            </label>
            <div className="flex gap-2">
              <div className="relative min-w-0 flex-1">
                <input
                  ...
                  className="w-full rounded-lg border border-[#5b21b6] bg-[#241339] px-3 py-2 pr-10 font-mono-gem text-xs text-[#f5f3ff] placeholder:text-[#c4b5fd]/50 outline-none transition-colors focus:border-[#7c3aed]/60"
                />
                <button
                  type="button"
                  onClick={() => setShowKey((s) => !s)}
                  aria-label={showKey ? "Ocultar llave" : "Mostrar llave"}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-[#c4b5fd] hover:text-[#f5f3ff]"
                >
              ...
              <button
                type="button"
                onClick={saveKey}
                className="ctrl-btn rounded-lg border border-[#7c3aed]/50 bg-[#7c3aed]/10 px-3.5 py-2 text-xs font-semibold text-[#7c3aed] hover:bg-[#7c3aed]/20"
              >
                Guardar
              </button>
            </div>
            <p className={`mt-2 text-[11px] leading-relaxed ${keySavedFlash ? "text-[#a855f7]" : "text-[#c4b5fd]"}`}>
              {keySavedFlash
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <div className="rounded-lg border border-[#5b21b6] bg-[#2e1065] px-3 py-2.5">
                <p className="font-mono-gem text-[10px] uppercase tracking-widest text-[#c4b5fd]">Feedback sonoro</p>
                <button type="button" onClick={toggleMute} className="ctrl-btn mt-1 flex items-center gap-1.5 text-xs font-semibold text-[#f5f3ff]">
                  {muted ? <SpeakerOffIcon size={14} className="text-[#c4b5fd]" /> : <SpeakerOnIcon size={14} className="text-[#a855f7]" />}
                  {muted ? "Silenciado" : "Activado"}
                </button>
              </div>
              <div className="rounded-lg border border-[#5b21b6] bg-[#2e1065] px-3 py-2.5">
                <p className="font-mono-gem text-[10px] uppercase tracking-widest text-[#c4b5fd]">Volumen sfx</p>
                <p className="mt-1 font-mono-gem text-xs font-semibold text-[#6d28d9]">SOUND_VOLUME = {SOUND_VOLUME}</p>
              </div>
            </div>

            <details className="group mt-4 rounded-lg border border-[#5b21b6] bg-[#2e1065]">
              <summary className="cursor-pointer select-none px-3 py-2.5 font-mono-gem text-[10px] uppercase tracking-widest text-[#c4b5fd] transition-colors hover:text-[#f5f3ff]">
                Prompt del sistema –¾
              </summary>
              <p className="border-t border-[#5b21b6] px-3 py-2.5 text-xs italic leading-relaxed text-[#f5f3ff]">
                —œ{SYSTEM_PROMPT}—
              </p>
            </details>
          </section>
